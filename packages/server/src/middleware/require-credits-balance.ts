...
export const requireCreditsBalance = createMiddleware<AuthenticatedEnv>(
  async (c, next) => {
    try {
      ...
      // This is a simple lauch-time gate: only start new work when the customer still has credits left. It does not reserve the full eventual cost of the request, so low-volume apps may tolerate small overspend on edge cases.
      if (creditsBalance <= 0) {
        return c.json(
          {
