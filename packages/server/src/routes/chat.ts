...
type EasycodeUIMessage = UIMessage<
  ChatMessageMetadata,
  never,
...
const submitSchema = z.object({
  ...
  messages: z
    .array(
      z.custom<EasycodeUIMessage>((value) => {
        return (
          value != null &&
          typeof value === "object" &&
          "id" in value &&
...
function hasPendingToolCalls(message: EasycodeUIMessage) {
...
const app = new Hono<AuthenticatedEnv>().post(
  ...
  async (c) => {
    ...
    const previousMessages = Array.isArray(session.messages)
      ? (session.messages as unknown as EasycodeUIMessage[])
      : [];
    ...
    const nextMessages = await validateUIMessages<EasycodeUIMessage>({
